...
//	private HashMap<Long, NeighborProxy> queuedConnectingNeighbors;
//	
//	private HashMap<Long, Long> queuedConnectingTime;
//
//	private HashMap<Long, NeighborProxy> queuedDisconnectingNeighbors;
//	
//	private HashMap<Long, Long> queuedDisconnectingTime;
//	
//	private HashMap<Long, Long> lastResponses;
...
//		this.queuedConnectingNeighbors = new HashMap<Long, NeighborProxy>();
//		this.queuedDisconnectingNeighbors = new HashMap<Long, NeighborProxy>();
//		this.queuedConnectingTime = new HashMap<Long, Long>();
//		this.queuedDisconnectingTime = new HashMap<Long, Long>();
//		this.lastResponses = new HashMap<Long, Long>();
...
//		int result;
//		long minDist, currentDist;
//		Iterator<Entry<Integer, NeighborProxy>> it;
//		NeighborProxy current, min;
//		HashMap<Integer, NeighborProxy> longRange;
//		long [] currentCoord, peerCoord;
//		
//		min = null;
//		peerCoord = peer.getPosition();
//		result = 0;
//		longRange = null;
//		int i = 0;
//		minDist = Long.MAX_VALUE;
//		while ( (min == null) || (min.getId() != peer.getVirtualEntity().getId()) ) {
//			System.err.println(i+"3"+min+" "+minDist);
//			result++;
//			minDist = Long.MAX_VALUE;
//			if (min == null) {
//				it = this.proxies.entrySet().iterator();
//				if (this.type == SolipsisProtocol.SMALLWORLD) {
//					longRange = this.smallWorldModule.getLinks();
//				}
//			} else {
//				it = this.getNeighbors(min);
////				System.err.println(i+"3.5");
//				if (this.type == SolipsisProtocol.SMALLWORLD) {
//					longRange = this.getLongRangeLinks(min);
//				}
//			}
////			System.err.println(i+"4");
//			while (it.hasNext()) {
//				current = (NeighborProxy)((Map.Entry<Integer, NeighborProxy>)it.next()).getValue();
//				currentCoord = current.getCoord();
//				currentDist = VirtualWorld.simpleDistance(currentCoord, peerCoord);
...
//			if (this.type == SolipsisProtocol.SMALLWORLD) {
//				it = longRange.entrySet().iterator();
//				while (it.hasNext()) {
//					current = (NeighborProxy)((Map.Entry<Integer, NeighborProxy>)it.next()).getValue();
//					currentCoord = current.getCoord();
//					currentDist = VirtualWorld.simpleDistance(currentCoord, peerCoord);
...
////		System.err.println(i+"6");
	...
	public int estimateHops (SolipsisProtocol peer, boolean loopback, boolean useLongRange) {
		...
		if (!loopback) {
			if (hops == -1) {
				hops = peer.estimateHops(this, true);//jumpingToPositionConnectionAlgorithm(this, true, 0, peer.getVirtualEntity().getCoord(), peer.getVirtualEntity().getId());
			}
		}
//		if (hops == -1) {
...
//		}
	...
	public void lookup(Lookup request) {
		Globals.totalUse++;
//		System.out.println(this.mainVirtualEntity.getId()+": Lookup (of "+request.getSource().getId()+")");
...
//				request.incrementHops();
		...
		if (localClosest != null) {
			...
			if (neighborMinDistance < currentDistance) {
				...
				//			System.out.println("destination=("+destination[0]+","+destination[1]+")," +request.getSource().getId()+"findNearest| id="+this.mainVirtualEntity.getId()+", coord=("+this.getPosition()[0]+","+this.getPosition()[1]+") says: my distance:"+currentDistance+", next=" + localClosest.getId()+" with d="+neighborMinDistance+" coord=("+localClosest.getCoord()[0]+","+localClosest.getCoord()[1]+")");
			} else {
				//			System.out.println(request.getSource().getId()+"current"+currentDistance);
				if(this.simpleLeftFromLine(destination, myPosition, neighborCoord)) {
					direction = VirtualWorld.LEFT;
				} else {
					direction = VirtualWorld.RIGHT;
					//				System.out.println(VirtualWorld.samePosition(this.getPosition(), destination));
				}
...
//			this.mainVirtualEntity.setCoord(VirtualWorld.randomCoord(VirtualWorld.XMAX), VirtualWorl);
...
//				System.out.println("Entity "+this.mainVirtualEntity.getId()+": "+sector[0].getId()+","+sector[1].getId());
...
//		if (this.isNeighbor(source.getVirtualEntity().getId())) {
	...
	private void processFoundMsg(Message msg) {
		...
		GeometricRegion line;
		Message recoverMsg;
//		System.out.println(this.mainVirtualEntity.getId()+": found "+((peer==null)?null:peer.getId())+" from "+msg.getSource());
		if (peer!=null) {
			if (!isNeighbor(peer)) {
				switch(peer.getQuality()) {
				case NeighborProxy.REGULAR:
//					this.queueConnectingNeighbor(peer);//
			...
			this.searchInProgress = false;
//			System.out.println(this.mainVirtualEntity.getId()+": found nothing from"+((SolipsisProtocol)Network.get(msg.getOriginAddress()).getProtocol(this.protocolId)).getVirtualEntity().getId());
		}
	}
